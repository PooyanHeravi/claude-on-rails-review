...
def format_files_grouped(files: list[str]) -> str:
...
def get_review_instructions(
    ...
    # Results instruction - output inline with markers (no file writes needed)
    pending_agents_list = ", ".join(f'"{a}"' for a in pending_agents)
    results_instruction = f'''

IMPORTANT: After ALL agents complete, output results in this EXACT format (do NOT write any files):

<!--REVIEW_RESULTS_START-->
{{"round_id": "{round_id}", "agents": {{"<agent_id>": {{"status": "pass", "issues": []}}}}}}
<!--REVIEW_RESULTS_END-->
...
{{
  "status": "pass" | "fail",
  "issues": [
    {{"file": "path/to/file.py", "line": 42, "severity": "critical|high|medium|low", "category": "...", "description": "..."}}
  ]
}}

Agents to report: [{pending_agents_list}]

STATUS CRITERIA:
- "fail" = ANY critical severity issue OR 2+ high severity issues
- "pass" = otherwise (medium/low are informational, report but don't fail)

SEVERITY GUIDE:
- critical: Security vulnerabilities, data loss, silent failures (CLAUDE.md non-negotiables)
- high: Bugs that will cause runtime errors, race conditions, resource leaks
- medium: Code smells, missing validation, inefficient patterns
...
#   "planning_state": str,           # "none" | "plan_needed" | "plan_requested" - subagent planning flow
#   "plan_round_id": str,            # UUID for the current planning round
# }
#
...
def save_review_state(
    ...
    Args:
        ...
        planning_state: Planning flow state ("none", "plan_needed", "plan_requested").
        plan_round_id: UUID for the current planning round.
    """
    state = {
...
def extract_results_from_transcript(transcript_path: str) -> dict | None:
    """Extract review results from transcript using markers.
...
def read_review_results(transcript_path: str) -> dict:
    """Read review results from transcript with retry logic.

    Args:
        transcript_path: Path to the transcript file.

    Retries up to 3 times with exponential backoff (0.1s, 0.2s, 0.4s)
    to handle cases where Claude hasn't output results yet.
    Returns {} if all attempts fail.
    """
    for attempt in range(3):
        result = extract_results_from_transcript(transcript_path)
        if result is not None:
            return result
        if attempt < 2:  # Don't sleep after last attempt
            sleep_time = RETRY_INITIAL_DELAY * (RETRY_BACKOFF_FACTOR ** attempt)
            log(f"Results extraction failed, retry {attempt + 1}/3 in {sleep_time}s")
...
def main() -> None:
    ...
    if is_new_session:
        ...
        planning_state = state.get("planning_state", "none")
        plan_round_id = state.get("plan_round_id", "")
    ...
    # -------------------------------------------------------------------------
    # Plan subagent flow for deep review failures
    # -------------------------------------------------------------------------
    if planning_state == "plan_needed":
        # Need to spawn plan agent - generate instructions
        log("Plan needed - returning plan agent instructions")

        # Read the failed review results from transcript
        results = read_review_results(transcript_path)

        # Extract code hunks for context
        ...
        # Reset planning state - Plan agent returns text, no file verification needed
        save_review_state(
            session_hash=session_hash,
            ...
            tier=old_tier,
            auto_continue_count=auto_continue_count,
            fail_count=0,  # Reset fail count after plan
            round_id="",
            passed_agents=[],
            completed=False,
            violation_history=violation_history,
            planning_state="none",  # Reset - no verification needed
            plan_round_id="",
        )
    ...
    if incremental_diff == 0 and len(incremental_files) == 0:
        # Check if a review round just completed (results in transcript with matching round_id)
        # This happens when Claude ran the agents and output the results
        if round_id:
            results = read_review_results(transcript_path)
            if results.get("round_id") == round_id:
                ...
                if all_agents_passed and required_agents:
                    ...
                    # Save updated state - clear planning_state since review passed
                    save_review_state(
                        session_hash=session_hash,
                        session_id=session_key,
                        last_total_diff=current_total_diff,
                        ...
                        completed=False,
                        violation_history=violation_history,
                        planning_state="none",  # Clear since review passed
                        plan_round_id="",
                    )
                    ...
                    if failed_agents and old_tier == "deep":
                        ...
                        # Set planning_state to trigger plan agent flow
                        save_review_state(
                            session_hash=session_hash,
                            session_id=session_key,
                            last_total_diff=current_total_diff,
                            ...
                            completed=False,
                            violation_history=violation_history,
                            planning_state="plan_needed",  # Trigger plan agent flow
                            plan_round_id=uuid.uuid4().hex[:ROUND_ID_LENGTH],
                        )
    ...
    if REVIEW_MODE == "agent":
        ...
        # Read previous review results from transcript (if any)
        results = read_review_results(transcript_path)
        if results.get("round_id") == round_id:
