...
# Set CLAUDE_HOOK_DEEP_AUTO_FIX to override DEEP_AUTO_FIX (same values)
...
# =============================================================================
# Exit Helpers — every exit MUST go through one of these two functions.
...
def allow_stop(reason: str = "") -> None:
    ...
    Use this when the hook is done and Claude should be allowed to stop.
    Produces NO output — the hook API interprets empty stdout + exit 0 as "allow".
    """
    if reason:
...
def block_with_message(reason: str) -> None:
    ...
    Use this when the hook wants Claude to keep going (e.g., to fix issues,
    auto-continue with review instructions, or report failures).
...
# The static audit test (test_exit_paths.py) validates this matches actual code.
EXIT_PATH_REGISTRY = {
    ...
    "all_passed_max":          {"type": "allow", "line_hint": "_handle_all_passed()"},
    "all_passed_continue":     {"type": "block", "line_hint": "_handle_all_passed()"},
    "deep_auto_fix":           {"type": "block", "line_hint": "_handle_deep_failure()"},
    "deep_plan_agent":         {"type": "block", "line_hint": "_handle_deep_failure()"},
    "deep_completed":          {"type": "allow", "line_hint": "main()"},
    "no_incremental_continue": {"type": "allow", "line_hint": "main()"},
    "no_incremental_default":  {"type": "allow", "line_hint": "main()"},
    "skip_max_continues":      {"type": "allow", "line_hint": "main()"},
    "skip_consecutive":        {"type": "allow", "line_hint": "main()"},
    "skip_continue":           {"type": "block", "line_hint": "main()"},
    "completed_agent":         {"type": "allow", "line_hint": "main()"},
    "max_continues_agent":     {"type": "allow", "line_hint": "main()"},
    "max_fails_agent":         {"type": "block", "line_hint": "main()"},
    "zero_diff_max_fails":     {"type": "block", "line_hint": "main()"},
    "review_instructions":     {"type": "block", "line_hint": "main()"},
    "api_max_continues":       {"type": "allow", "line_hint": "main()"},
    "api_clean":               {"type": "block", "line_hint": "main()"},
    "api_violations":          {"type": "block", "line_hint": "main()"},
    "fatal_error":             {"type": "allow", "line_hint": "__main__"},
}
...
def extract_pre_review_context(transcript_path: str) -> str:
    ...
    - The last user message (truncated to 200 chars)
    - The last ~5 assistant tool calls (tool name + file_path)

    Returns a formatted context string to append to continue/resume messages,
    ...
    try:
        with open(transcript_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if role == "assistant" or event.get("message", {}).get("role") == "assistant":
                    tool_uses = _extract_tool_uses(event)
                    for tool_data in tool_uses:
                        tool_name = tool_data.get("name") or tool_data.get("tool_name")
                        if not tool_name:
...
{results_schema}'''
    ...
    # For deep reviews, behavior depends on DEEP_AUTO_FIX setting
    if tier == "deep":
        ...
        if effective_auto_fix == "none":
            ...
            qualifying = _severities_at_or_above(effective_auto_fix)
...
def _handle_all_passed(
    *,
    ...
    context_str = extract_pre_review_context(transcript_path) if transcript_path else ""
    log(f"Review passed - issuing auto-continue [Auto-continue {auto_continue_count} of {MAX_AUTO_CONTINUES}]")
    block_with_message(f"All reviews passed. [Auto-continue {auto_continue_count} of {MAX_AUTO_CONTINUES}] {continue_msg}{context_str}")
...
def _handle_deep_failure(
    *,
    ...
    """Handle deep review failure.

    If DEEP_AUTO_FIX is "none", triggers the plan agent and stops.
    Otherwise, instructs Claude to spawn a subagent to fix qualifying issues
    by severity, report the rest, and resume.
    """
    total_issues = sum(
        len(data.get("issues", []))
    ...
    if effective_auto_fix != "none":
        # Auto-fix mode: filter issues by severity and instruct subagent fix
        qualifying_severities = _severities_at_or_above(effective_auto_fix)
        ...
        fixable = [i for i in all_issues if i.get("severity", "").lower() in qualifying_severities]
        reportable = [i for i in all_issues if i.get("severity", "").lower() not in qualifying_severities]
        ...
        log(f"Deep auto-fix mode ({effective_auto_fix}): {len(fixable)} fixable, {len(reportable)} report-only")
    ...
    block_with_message(f"⚠️ DEEP REVIEW FAILED: {len(failed_agents)} agent(s) found {total_issues} issue(s).\n\n{plan_instructions}")
...
def main() -> None:
    ...
    if incremental_diff == 0 and len(incremental_files) == 0:
            ...
            if results.get("round_id") == round_id:
                ...
                if all_agents_passed and required_agents:
                    ...
                    if failed_agents and old_tier == "deep":
                        accumulated_fail_count = fail_count + len(failed_agents)
                        if accumulated_fail_count >= MAX_FAIL_RETRIES:
                            log(f"Max fail retries in zero-diff path ({accumulated_fail_count} >= {MAX_FAIL_RETRIES}) - allowing stop")
                            save_review_state(
                                session_hash=session_hash,
                                session_id=session_key,
                                last_total_diff=current_total_diff,
                                ...
                                tier=old_tier,
                                auto_continue_count=auto_continue_count,
                                fail_count=accumulated_fail_count,
                                round_id=round_id,
                                passed_agents=passed_agents,
                                completed=True,
                                violation_history=violation_history,
                            ...
                            context_str = extract_pre_review_context(transcript_path)
                            block_with_message(f"⚠️ Review retries exhausted ({accumulated_fail_count} failures, max {MAX_FAIL_RETRIES}). Automated reviews found persistent issues that could not be resolved automatically.{context_str}\n\nReport this to the user, then resume your prior task.")
        ...
        if auto_continue_count > 0:
            allow_stop(f"No new changes after auto-continue {auto_continue_count}")
    ...
    if tier == "skip":
        ...
        if auto_continue_count >= MAX_AUTO_CONTINUES:
            ...
            context_str = extract_pre_review_context(transcript_path)
            block_with_message(f"Minimal changes ({abs(incremental_diff)} chars, {len(incremental_files)} files). [Auto-continue {auto_continue_count} of {MAX_AUTO_CONTINUES}] {continue_msg}{context_str}")
    ...
    if REVIEW_MODE == "agent":
        ...
        if completed:
            ...
            # Reset completed flag so future sessions start clean
            save_review_state(
                session_hash=session_hash,
                session_id=session_key,
                last_total_diff=current_total_diff,
        ...
        if auto_continue_count >= MAX_AUTO_CONTINUES:
            ...
            allow_stop("Max auto-continues reached (agent mode)")
        ...
        if fail_count >= MAX_FAIL_RETRIES:
            ...
            context_str = extract_pre_review_context(transcript_path)
            block_with_message(f"⚠️ Review retries exhausted ({fail_count} failures, max {MAX_FAIL_RETRIES}). Automated reviews found persistent issues that could not be resolved automatically.{context_str}\n\nReport this to the user, then resume your prior task.")
        ...
        if not violations:
            ...
            continue_msg = _get_continue_message(auto_continue_count)
            context_str = extract_pre_review_context(transcript_path)
            block_with_message(f"Code review passed ({model_name}). No violations found.\n\n[Auto-continue {auto_continue_count} of {MAX_AUTO_CONTINUES}] {continue_msg}{context_str}")

        # Violations found
        violation_text = "\n".join(violations)
        log(f"Violations found (reviewed by {model_name})")
        block_with_message(f"VIOLATIONS FOUND (reviewed by {model_name}):\n{violation_text}\n\nPlease fix these before proceeding.")
...
if __name__ == "__main__":
    try:
        ...
        # Always exit cleanly — use raw sys.exit here since log() may itself be broken
        try:
            log(f"FATAL ERROR: {e}")
        except Exception:
            pass
        sys.exit(0)  # noqa: allow_stop not used — log() may be unavailable
